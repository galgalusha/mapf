...
def get_shortest_path(agent_idx: int, start: Coord, grid: Grid, agents: list[Agent], dist_tables, occupied_now) -> list[Coord]:
    ...
    while p[-1] != goal:
        ...
        for nxt in neighbors(grid, v):
            ...
            is_occupied = nxt in occupied_now
            
            if d < best_dist or (d == best_dist and not is_occupied and best_occ):
                best_dist, best_occ, best_nxt = d, is_occupied, nxt
        if best_nxt is None:
            break
        p.append(best_nxt)
    return p
...
if __name__ == "__main__":
    ...
    # scene = get_scenario('./assets/two_rooms_narrow_door_2.scen')
