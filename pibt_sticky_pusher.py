...
class PIBT_FUNC_RESULT(Enum):
    Failed = 1
    Pushed_Another_Agent = 2
    Clear_Pass = 3
...
def func_pibt(
    ...
    for coord_idx, coord in enumerate(candidate_coords):
        ...
        return PIBT_FUNC_RESULT.Pushed_Another_Agent if need_to_push else PIBT_FUNC_RESULT.Clear_Pass

    occupied_next[q_from[i]] = i
    q_to[i] = q_from[i]
    return PIBT_FUNC_RESULT.Failed
...
def set_new_config(
    ...
    for i in order:
        if q_to[i] is None:
            ...
            if result == PIBT_FUNC_RESULT.Failed:
                return None
            agents[i].data[WAS_PUSHER] = (result == PIBT_FUNC_RESULT.Pushed_Another_Agent)
...
def calculate_priority(agent: Agent, distance_from_goal: int):
    pusher_bonus = -1 if agent.data[WAS_PUSHER] else 0
    return (pusher_bonus, -distance_from_goal, agent.id)
...
def solve(grid: Grid, agents: list[Agent]) -> Optional[Plan]:
    ...
    for step_itr in range(max_steps):
        ...
        order = sorted(
            range(n_agents),
            key=lambda i: calculate_priority(agents[i], d_get(dist_tables, i, current[i])),
        )
...
if __name__ == "__main__":
    ...
    # grid: Grid = get_grid('./assets/warehouse-10-20-10-2-1.map')
    # scene = get_scenario('./assets/warehouse-10-20-10-2-1.map-scen-random/scen-random/warehouse-10-20-10-2-1-random-1.scen', 40)

    grid: Grid = get_grid('./assets/2_rooms_2_doors_5x5.map')
    scene = get_scenario('./assets/2_rooms_2_doors_5x5.scen')
