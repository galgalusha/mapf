...
def calculate_priority(goal: Coord, grid_shape: tuple[int, int]) -> int:
    H, W = grid_shape
    r, c = goal
    border_dist = min(r, H - 1 - r, c, W - 1 - c)
    return -10000 * border_dist
...
def solve(grid: Grid, agents: list[Agent]) -> Optional[Plan]:
    ...
    max_steps = min(rows * cols * 8, 50000)

    # Pre-calculate static priorities based on goal position
    priorities = [calculate_priority(agent.target, grid.shape) for agent in agents]
    ...
    for time in range(max_steps):
        ...
        order = sorted(
            range(n_agents),
            key=lambda i: (-priorities[i], -d_get(dist_tables, i, current[i]), agents[i].id),
        )

        if time > 300 and time < 304:
            print(f'--- {time} ---')
            bad_agents = [i for i in range(n_agents) if agents[i].target != current[i]]
            for agent_i in bad_agents:
                a_order = order.index(agent_i)
                a_priority = priorities[agent_i]
                print(f'agent {agent_i} current {current[agent_i]} target {agents[agent_i].target} order: {a_order} priority: {a_priority}')
...
if __name__ == "__main__":
    ...
    scene = get_scenario('./assets/warehouse-10-20-10-2-1.map-scen-random/scen-random/warehouse-10-20-10-2-1-random-1.scen', 35)
